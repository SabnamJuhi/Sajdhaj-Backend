...
exports.getAdminOrderHistory = async (req, res) => {
  try {
    ...
    const formattedOrders = orders.rows.map((order) => {
      ...
      const items = orderJson.OrderItems?.map((item) => {
        ...
        return {
          ...
          gst: {
            ...
            // amount: Number(item.gstAmount) || 0, // Uncomment if you have this field
            // priceWithGst: Number(item.finalPrice) || totalFinalAmount, // Uncomment if you have this field
          },
      ...
      return {
        ...
        historyDetails: {
          ...
          cancelledOn: order.cancelledAt,
          refundedOn: order.refundedAt,
          returnedOn: order.returnedAt,
          orderAge: Math.ceil((new Date(order.completedAt || order.deliveredAt || new Date()) - new Date(order.createdAt)) / (1000 * 60 * 60 * 24)) + " days",
          
          // Additional history insights
          customerSavings: order.totalDiscount,
          effectiveDiscountRate: order.totalOriginalAmount > 0 
            ? ((order.totalDiscount / order.totalOriginalAmount) * 100).toFixed(2) + '%'
            : '0%',
          orderDuration: order.deliveredAt || order.completedAt 
            ? Math.ceil((new Date(order.deliveredAt || order.completedAt) - new Date(order.createdAt)) / (1000 * 60 * 60 * 24)) + " days"
            : 'N/A'
        }
      };
    });
    ...
    const statistics = {
      totalOrders: orders.count,
      totalRevenue: formattedOrders.reduce((sum, order) => sum + order.orderDetails.totalAmount, 0),
      totalOriginalValue: formattedOrders.reduce((sum, order) => sum + order.orderDetails.totalOriginalAmount, 0),
      totalDiscountGiven: formattedOrders.reduce((sum, order) => sum + order.orderDetails.totalDiscount, 0),
      averageOrderValue: orders.count > 0 
        ? formattedOrders.reduce((sum, order) => sum + order.orderDetails.totalAmount, 0) / orders.count 
        : 0,
      ...
      financialBreakdown: {
        completedRevenue: formattedOrders
          .filter(o => ['delivered', 'completed'].includes(o.orderDetails.status))
          .reduce((sum, o) => sum + o.orderDetails.totalAmount, 0),
        cancelledValue: formattedOrders
          .filter(o => o.orderDetails.status === 'cancelled')
          .reduce((sum, o) => sum + o.orderDetails.totalAmount, 0),
        refundedValue: formattedOrders
          .filter(o => ['refunded', 'returned'].includes(o.orderDetails.status))
          .reduce((sum, o) => sum + o.orderDetails.totalAmount, 0),
      }
    };
